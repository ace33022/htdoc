	...
	var result = function(connection) {
		...
		this.doSelectBetweenTrnDateOrderByTrnDate = function(beginDate, endDate) {
			...
			try {
				...
				prepareStatement.setString(1, beginDate);
				prepareStatement.setString(2, endDate);
				
				this.setSelectResult(prepareStatement.executeQuery(), result);
			}
			finally {
		...
		this.doDeleteByTrnDate = function(trnDate) {
			...
			try {
				...
				prepareStatement.setString(1, trnDate);
				
				if (prepareStatement.executeUpdate() > 0) result = true;
			}
			finally {
